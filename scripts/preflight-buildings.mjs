...
const warningId = item => ['buildings', item.code, item.sheet, item.row, item.record_id, item.building_key, item.source_text, item.parsed_name ?? '', item.required_level ?? '', item.required_stage ?? ''].join('|')
...
for (const [index, row] of progression.entries()) {
  ...
  for (const value of values) {
    ...
    if (!resolved) issue(findings, 'Warning', 'unresolved_prerequisite', `Prerequisite ${parsed.source_text} could not be mapped to a canonical building_key.`, { ...item, warning_id: warningId({ ...item, code: 'unresolved_prerequisite' }) }, 'Resolve editorially or preserve the original source text without creating a fictitious building.')
