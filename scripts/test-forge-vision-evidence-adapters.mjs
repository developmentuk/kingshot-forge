...
const rowActiveAcceptance = { id: '66666666-6666-4666-8666-666666666666', owner_user_id: ownerId, purpose: 'scan_source', upload_purpose: 'Preview-only OCR acceptance review', mime_type: 'image/png', byte_length: 1234, verified_at: '2026-07-24T16:02:00.000Z', deleted_at: null, legal_hold: false }
...
const activeAcceptance = await repository.listActiveAcceptanceEvidenceForOwner(ownerId, 5)
assert.deepEqual(activeAcceptance, [{ evidenceId: rowActiveAcceptance.id, uploadedAt: rowActiveAcceptance.verified_at, mimeType: 'image/png', byteLength: 1234, status: 'active' }])
assert.deepEqual(captured.queries.at(-1), { table: 'vision_evidence_images', filters: { owner_user_id: ownerId, purpose: 'scan_source', upload_purpose: 'Preview-only OCR acceptance review', deleted_at: null, legal_hold: false }, limit: 5 })
for (const forbidden of ['path', 'bucket', 'sha256', 'signedUrl']) assert.equal(forbidden in activeAcceptance[0], false)
...
function query(table, rows, capture) { const state = { table, filters: {} }; const builder = { select() { return builder }, eq(key, value) { state.filters[key] = value; return builder }, is(key, value) { state.filters[key] = value; return builder }, order() { return builder }, limit(value) { capture.queries = capture.queries ?? []; capture.queries.push({ table, filters: { ...state.filters }, limit: value }); return Promise.resolve({ data: table === 'vision_evidence_images' && state.filters.upload_purpose === 'Preview-only OCR acceptance review' ? [rowActiveAcceptance] : [], error: null }) }, insert(row) { capture.inserts.push({ table, row }); return Promise.resolve({ data: null, error: null }) }, update(row) { capture.updates.push({ table, row }); return builder }, maybeSingle() { return Promise.resolve({ data: state.filters.sha256 ? rowEvidence : state.filters.id === evidenceId ? rowEvidence : state.filters.id === intentId ? rowIntent : null, error: null }) }, single() { return Promise.resolve({ data: { id: state.filters.id ?? evidenceId }, error: null }) } }; return builder }
