...
const compressed = gzipSync(lockfile, { level: 9 })
const encoded = compressed.toString('base64')
const chunkSize = 1200
const chunkCount = Math.ceil(encoded.length / chunkSize)
const digest = createHash('sha256').update(lockfile).digest('hex')

console.log(`LOCKFILE_BEGIN bytes=${lockfile.length} gzip=${compressed.length} base64=${encoded.length} chunks=${chunkCount} sha256=${digest}`)
