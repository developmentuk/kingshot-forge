...
Focused tests cover strict projection behavior, exhaustive recognised trust-state mapping and fail-closed unknown states, forbidden-field injection, content-hash stability under record/timestamp order changes and sensitivity to public content, source-fingerprint sensitivity to catalogue/media changes and stability under key/record/media ordering, immutable rollback reproduction and altered/missing/cross-dataset target rejection, 55-record/430-level preservation, manifest/file checksums, 111 unique mappings, exact six-placeholder metadata, extra/duplicate media rejection, and the complete adversarial nested matrix for null/scalar/array levels, missing/null/malformed bonuses, known effects, aliases, footprint, unlock, upgrade, maximum effects, nullable scalars, and missing or extra nested keys. The suite proves the shared validator is used by publication, the published-only loader and Search; it also structurally proves complete SQL guards precede locking and mutation, the authoritative clock is captured once after the lock, same-millisecond monotonic advancement, idempotent retry preservation, rollback timestamp propagation, inactive production routes/registries, fixture isolation and clean-build artifact exclusion. A production dependency boundary test also verifies that pinned `sharp` is installed from `dependencies` and that the production OCR/API import graph loads successfully. SQL and concurrency coverage is structural in this environment: the migration was not executed against a disposable database, and actual concurrent PostgreSQL execution remains a mandatory Phase 2A acceptance test. Full required repository validation results are recorded in the draft PR and final delivery report.
