...
- Public WebP derivatives: 111, 1,608,830 bytes (approximately 1.53 MiB), plus one shared 720×720 designed placeholder.
...
- Manifest: checked in server-side with source record, private filename/checksum, slug-safe derivative path/checksum, dimensions, role/level variant and alt text. Public fixture records contain only public URLs and alt text.
...
`oasis-acceptance.html` is a Vite development entry point using the sanitised checked-in projection fixture and a hash router. It supports catalogue/detail review, text and rarity filtering, all level tables, WebP artwork and placeholder states, responsive desktop/mobile CSS, and links to the existing Island Chest Route Optimizer. It is not imported by `src/main.tsx`, and the production app has no Oasis route.
...
Focused tests cover strict projection behavior, forbidden-field injection, 55-record/430-level preservation, manifest/file checksums, 111 unique mappings, six placeholders, size budgets, loader failure modes, migration structure, inactive production routes/registries, fixture isolation and Search projection. Full required repository validation results are recorded in the draft PR and final delivery report.
