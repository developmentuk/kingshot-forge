...
## D1A4 executable incident boundary

D1A4 replaces the literal checkpoint with a structured exact manifest and separates `activationSourceSha` from the future cleanup execution SHA. The server-only gateway reads only the two exact intents, confirms the private bucket, migration ledger and governed policy/grant/RLS/constraint state, uses exact provider object operations, appends one bounded abandonment audit, and deletes intents only after confirmed object absence. The audit invariant is seven before cleanup, eight after abandonment, and eight at completion.

D1A5 makes the audit invariant explicit as total Vision audits 7/8/8 and incident-specific audits 3/4/4, preserving the original four C3 events. Migration activation is supplied by a separately captured read-only ledger result using actual migration names and combined with exact schema evidence; repository filenames are not used as live ledger versions.

D1A6 removes all PostgREST access to PostgreSQL system views from the cleanup gateway. Governance is supplied through an exact structured operator capture and validated against the manifest before the live gateway is constructed.
