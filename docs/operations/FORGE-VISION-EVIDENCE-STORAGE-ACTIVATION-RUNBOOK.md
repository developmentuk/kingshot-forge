...
5. Apply the frozen storage migration, then the prepared governance correction, through the separately approved migration workflow.
...
9. Verify exact metadata, SHA-256, dimensions, signed upload/read expiry and audit events.
10. Exercise an expired URL, cross-owner denial, malformed path, unsupported MIME, oversized file, duplicate hash and metadata-failure containment.
...
Capture exact commit and migration hashes, project binding, migration ledger, bucket configuration, policies, anonymous denial, authenticated owner/reviewer matrix, signed URL expiry, synthetic object digest, zero-object post-cleanup counts, retained audit metadata, logs without secrets/URLs/image bytes, and the owner approval. Do not claim activation or legal compliance without this evidence.
