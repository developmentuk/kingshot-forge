...
responses. The runtime credential destination is pinned to the exact MightPulse
API origin. The server enforces a persisted 60-minute automatic freshness
boundary, a persisted five-minute minimum manual-refresh interval, an
authenticated per-instance attempt throttle, a bounded 45-second provider
timeout and safe error mapping. MightPulse Town Center maps only to
`town_center_level`; generic `player_level`, unsupported legacy display fields
and existing verification state are preserved during same-player revalidation.
...
primary linked-player runtime path. Operations provider lookup is read-only;
provider-backed administrator apply is deferred until an owner-approved atomic
admin-link contract can preserve Town Center and verification provenance. The
existing manual/community-verification administrator path remains unchanged.
No migration, Supabase change, deployment or production activation is part of this record. See
