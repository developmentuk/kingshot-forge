...
import { KINGSHOT_PROFILE_V1_REGIONS, KINGSHOT_PROFILE_V2_REGIONS, KINGSHOT_PROFILE_V3_REGIONS, type KingshotProfileRegionConfig } from '../../shared/domains/player-identity/kingshotProfileMapping.js'
...
export function profileRegionBindings(version: 'v1' | 'v2' | 'v3' = 'v2'): VisionRegionBinding[] {
  const regions = version === 'v1' ? KINGSHOT_PROFILE_V1_REGIONS : version === 'v3' ? KINGSHOT_PROFILE_V3_REGIONS : KINGSHOT_PROFILE_V2_REGIONS
  const mappingId = version === 'v1' ? 'account-linking-kingshot-profile-v1' : version === 'v3' ? KINGSHOT_PROFILE_V3_MAPPING_VERSION : KINGSHOT_PROFILE_V2_MAPPING_ID
  return regions.map((region, index) => ({ id: `${mappingId}-${region.key}`, regionKey: region.key, label: region.label, role: 'source', anchorRules: { layout: 'kingshot-profile-card', psm: region.psm, characterWhitelist: region.characterWhitelist, observation: region.observation }, sortOrder: index, x: region.x, y: region.y, width: region.width, height: region.height }))
}
