...
import { suggestKingshotClipboardMode } from '../render-engine'
...
function ArtworkCard({ item, favourite, onFavourite, onPreview, onCopy, onReact }: { item: GalleryRecord; favourite: boolean; onFavourite: () => void; onPreview: () => void; onCopy: () => void; onReact: (reaction: CommunityArtReactionType) => void }) {
  return <article className="art-library-card"><div className="art-library-card__heading"><div><span className="art-library-card__category">{item.category}</span><h2>{item.title}</h2></div><button type="button" className={favourite ? 'favourite-button favourite-button--active' : 'favourite-button'} onClick={onFavourite} aria-label={favourite ? 'Remove from favourites' : 'Add to favourites'} aria-pressed={favourite}>{favourite ? '★' : '☆'}</button></div><p className="art-library-card__description">{item.description}</p><p className="art-library-card__creator">Created by {item.attribution || 'Anonymous'}</p><button type="button" className="art-card-preview-button" onClick={onPreview} aria-label={`Preview ${item.title}`}><KingshotArtRenderer artwork={item.artworkText} compact sourceContext={item.sourceContext} maxLines={8} /><span>Tap to view full artwork</span></button><div className="art-library-card__metadata"><span>{item.characterCount} characters</span><span>{item.sizeClass}</span><span className={item.compatibilityStatus === 'verified' ? 'art-status art-status--tested' : 'art-status art-status--testing'}>{compatibilityLabel(item.compatibilityStatus)}</span></div><ReactionControls item={item} onReact={onReact} /><div className="art-library-card__actions"><button type="button" className="button button--secondary" onClick={onPreview}>Preview</button><button type="button" className="button button--primary" onClick={onCopy}>Copy Art</button></div></article>
}
...
function ArtworkPreview({ item, favourite, onClose, onFavourite, onCopy, onReact }: { item: GalleryRecord; favourite: boolean; onClose: () => void; onFavourite: () => void; onCopy: () => void; onReact: (reaction: CommunityArtReactionType) => void }) {
  ...
  return <div className="art-preview-modal" role="dialog" aria-modal="true" aria-labelledby="art-preview-title">
    ...
    <div className="art-preview-modal__panel" ref={panelRef}>
      ...
      <div className={`art-preview-modal__content art-preview-modal__content--${mode}`}><div className={`art-preview-stage art-preview-stage--${zoom === 'fit' ? 'fit' : 'zoomed'}`} style={previewStyle}><KingshotArtRenderer artwork={item.artworkText} mode={mode} sourceContext={item.sourceContext} fitMode={zoom === 'fit' && mode === 'kingshot' ? 'contain' : undefined} labelledBy="art-preview-title" /></div></div>
...
function SubmissionForm({ signedIn, onSignIn, onSubmitted }: { signedIn: boolean; onSignIn: () => void; onSubmitted: (record: CommunityArtRecord) => void }) {
  ...
  const [sourceContext, setSourceContext] = useState<'authored' | 'kingshot-clipboard'>('kingshot-clipboard')
  ...
  useEffect(() => { if (error) { const errorElement = document.querySelector<HTMLElement>('.art-studio-submission-shell .error-state'); if (errorElement) { errorElement.tabIndex = -1; errorElement.focus({ preventScroll: true }); errorElement.scrollIntoView({ block: 'nearest', behavior: 'smooth' }) } } }, [error])
