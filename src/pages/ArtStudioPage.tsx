...
import { bytesToBase64, inspectSourceText, sha256Bytes, type DetectedLineEnding, type IngestionMode } from '../../shared/domains/art-studio/sourceEvidence'
...
function ArtworkPreview({ item, favourite, onClose, onFavourite, onCopy, onReact }: { item: GalleryRecord; favourite: boolean; onClose: () => void; onFavourite: () => void; onCopy: () => void; onReact: (reaction: CommunityArtReactionType) => void }) {
  ...
  return <div className="art-preview-modal" role="dialog" aria-modal="true" aria-labelledby="art-preview-title"><button type="button" className="art-preview-modal__backdrop" onClick={onClose} aria-label="Close preview" /><div className="art-preview-modal__panel"><div className="art-preview-modal__header"><div><span className="art-library-card__category">{item.category}</span><h2 id="art-preview-title">{item.title}</h2></div><button type="button" className="art-preview-modal__close" onClick={onClose} aria-label="Close artwork preview">×</button></div><p className="art-library-card__creator">Created by {item.attribution || 'Anonymous'}</p><div className="art-preview-toolbar" aria-label="Artwork preview controls"><div className="art-preview-toolbar__group" role="group" aria-label="Preview mode"><button type="button" className={mode === 'kingshot' ? 'art-preview-control art-preview-control--active' : 'art-preview-control'} aria-pressed={mode === 'kingshot'} onClick={() => setMode('kingshot')}>In-game view</button><button type="button" className={mode === 'studio' ? 'art-preview-control art-preview-control--active' : 'art-preview-control'} aria-pressed={mode === 'studio'} onClick={() => setMode('studio')}>Studio view</button></div><label className="art-preview-zoom">Zoom<select value={zoom} onChange={(event) => setZoom(event.target.value === 'fit' ? 'fit' : Number(event.target.value) as PreviewZoom)}><option value="fit">Fit</option><option value="0.75">75%</option><option value="1">100%</option><option value="1.25">125%</option><option value="1.5">150%</option></select></label></div><div className={`art-preview-modal__content art-preview-modal__content--${mode}`}><div className="art-preview-stage" style={previewStyle}><KingshotArtRenderer artwork={item.artworkText} mode={mode} labelledBy="art-preview-title" /></div></div><div className="art-preview-modal__details"><span>{item.characterCount} characters</span><span>{item.lineCount} lines</span><span>{item.sizeClass}</span><span>{compatibilityLabel(item.compatibilityStatus)}</span><span>{item.source}</span></div><ReactionControls item={item} onReact={onReact} /><div className="art-preview-modal__actions"><button type="button" className="button button--secondary art-modal-favourite" onClick={onFavourite}>{favourite ? '★ Saved' : '☆ Favourite'}</button><button type="button" className="button button--primary" onClick={onCopy}>Copy Artwork</button></div></div></div>
}
...
function SubmissionForm({ signedIn, onSignIn, onSubmitted }: { signedIn: boolean; onSignIn: () => void; onSubmitted: (record: CommunityArtRecord) => void }) {
  ...
  return <div className="art-studio-submission-shell"><div className="art-studio-submission-intro"><p className="eyebrow">Community contribution</p><h2>Submit copyable chat art</h2><p>Upload a .txt file for exact source-byte evidence, or paste/type text received by the browser.</p></div><form className="art-studio-form" onSubmit={submit} noValidate>{error && <div className="error-state" role="alert">{error}</div>}<label htmlFor="submission-title">Artwork title<input id="submission-title" required maxLength={ART_STUDIO_TEXT_LIMITS.maximumTitleCharacters} value={title} onChange={(event) => setTitle(event.target.value)} /></label><label htmlFor="submission-description">Description<textarea id="submission-description" maxLength={ART_STUDIO_TEXT_LIMITS.maximumDescriptionCharacters} value={description} onChange={(event) => setDescription(event.target.value)} /></label><label htmlFor="submission-category">Category<select id="submission-category" value={category} onChange={(event) => setCategory(event.target.value as CommunityArtCategory)}>{COMMUNITY_ART_CATEGORIES.filter((item): item is CommunityArtCategory => item !== 'All').map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label htmlFor="submission-tags">Tags <span>(optional, comma separated)</span><input id="submission-tags" value={tags} onChange={(event) => setTags(event.target.value)} /></label><label htmlFor="submission-ingestion-mode">Source input<select id="submission-ingestion-mode" aria-label="Source input" value={ingestionMode} onChange={(event) => { const next = event.target.value as Exclude<IngestionMode, 'legacy_import'>; setIngestionMode(next); if (next !== 'file_upload') { setFileEvidence(null); setRawBytesBase64(''); setArtworkText('') } }}><option value="file_upload">Upload text file (exact bytes)</option><option value="text_paste">Paste or type artwork</option></select></label>{ingestionMode === 'file_upload' ? <><label htmlFor="submission-file">Upload text file<input id="submission-file" type="file" accept=".txt,text/plain" onChange={selectSource} /></label>{fileEvidence && <dl className="art-studio-source-evidence"><div><dt>File</dt><dd>{fileEvidence.filename}</dd></div><div><dt>Bytes</dt><dd>{fileEvidence.bytes}</dd></div><div><dt>SHA-256</dt><dd>{fileEvidence.sha256}</dd></div><div><dt>Line endings</dt><dd>{fileEvidence.lineEnding} · {fileEvidence.crlfCount} CRLF · {fileEvidence.lfCount} LF</dd></div><div><dt>Text</dt><dd>{fileEvidence.lineCount} lines · {fileEvidence.codePoints} code points · UTF-8</dd></div></dl>}<label>Decoded preview<pre className="art-studio-artwork-editor" aria-label="Decoded uploaded artwork">{artworkText}</pre></label></> : <><label htmlFor="submission-artwork">Paste or type artwork<textarea id="submission-artwork" className="art-studio-artwork-editor" required spellCheck="false" value={artworkText} onChange={(event) => setArtworkText(event.target.value)} placeholder={'  /\\_/\\\n ( o.o )\n  > ^ <'} /></label><small>Text entered in the browser. Browsers may normalize line endings; Forge preserves exactly what it receives and does not claim equality with an external file.</small></>}<div className="art-studio-artwork-count" aria-live="polite"><span>{countArtworkCharacters(artworkText)} / {ART_STUDIO_TEXT_LIMITS.maximumCharacters} characters</span><span>{new TextEncoder().encode(artworkText).byteLength} UTF-8 bytes</span><span>{countArtworkLines(artworkText)} / {ART_STUDIO_TEXT_LIMITS.maximumLines} lines</span></div><fieldset><legend>Attribution</legend><label><input type="radio" checked={attributionType === 'profile'} onChange={() => setAttributionType('profile')} /> Use my Forge display name</label><label><input type="radio" checked={attributionType === 'custom'} onChange={() => setAttributionType('custom')} /> Use a custom creator name</label>{attributionType === 'custom' && <input aria-label="Custom creator name" value={attributionName} maxLength={120} onChange={(event) => setAttributionName(event.target.value)} />}<label><input type="radio" checked={attributionType === 'anonymous'} onChange={() => { setAttributionType('anonymous'); setAttributionName('') }} /> Show as anonymous</label></fieldset><label className="art-studio-check"><input type="checkbox" checked={ownership} onChange={(event) => setOwnership(event.target.checked)} /> I own this artwork or have permission to submit it.</label><label className="art-studio-check"><input type="checkbox" checked={guidelines} onChange={(event) => setGuidelines(event.target.checked)} /> I agree to the Forge community guidelines.</label><div className="art-studio-form-actions"><button type="button" className="button button--secondary" onClick={() => setPreview((current) => !current)}>{preview ? 'Return to editing' : 'Preview artwork'}</button><button type="button" className="button button--secondary" onClick={() => setArtworkText('')}>Clear artwork</button><button type="submit" className="button button--primary" disabled={saving}>{saving ? 'Submitting…' : 'Submit for review'}</button></div>{preview && <div className="art-studio-submission-preview"><p className="eyebrow">In-game preview</p><KingshotArtRenderer artwork={artworkText} /><p>Characters: {countArtworkCharacters(artworkText)} · Lines: {countArtworkLines(artworkText)}</p><small>Calibrated from verified Kingshot screenshots. Rendering can still vary slightly by device and OS.</small></div>}</form></div>
}
