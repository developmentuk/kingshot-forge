...
function normalise(records: unknown[]): Building[] {
  ...
  records.forEach((raw) => { if (!raw || typeof raw !== 'object') return; const r = raw as Record<string, unknown>; const key = text(r.building_key, text(r.key, text(r.name, 'building').toLowerCase().replace(/\s+/g, '-'))); const current = map.get(key) ?? { key, name: text(r.building_name, text(r.name, key)), category: text(r.category, 'Buildings'), description: text(r.description, 'Verified building progression and upgrade effects.'), maxLevel: number(r.standard_max_level ?? r.max_level), truegold: Boolean(r.truegold_supported ?? r.truegold), progression: [], source: text(r.source_url, text(r.source)) }; if (r.record_id || r.level_label || r.level || r.base_level) current.progression.push(r); map.set(key, current) })
