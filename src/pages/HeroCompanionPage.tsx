...
function tierClassName(tier: HeroTier | null) {
  return `hero-companion-rating hero-companion-rating--${String(
    tier || 'na',
  )
    .replace('+', '-plus')
    .toLowerCase()}`
...
function HeroPortrait({
  ...
  return (
    <div
      className={`hero-companion-portrait${
        compact ? ' hero-companion-portrait--compact' : ''
      }`}
    >
      {hero.portrait_url ? (
        <img
    ...
    <main className="hero-companion-page hero-companion-page--detail">
      ...
      <section className="hero-companion-hero">
        <div className="hero-companion-hero__art">
          ...
          <span className="hero-companion-generation">
        ...
        <div className="hero-companion-hero__content">
          ...
          <p className="hero-companion-lead">
            {hero.description ||
              'A full Companion overview for this hero has not yet been published.'}
          </p>
      ...
      <section className="hero-companion-summary" aria-label="Hero summary">
        ...
        <article>
          <span>Rarity</span>
          <strong>{formatLabel(hero.rarity)}</strong>
        </article>
        <article>
      ...
      <section className="hero-companion-section">
        ...
        <div className="hero-companion-ratings">
          <RatingCard
            label="Rally"
            value={hero.rally_tier}
            description="Value when leading an alliance rally."
          />
          <RatingCard
            label="Garrison"
            value={hero.garrison_tier}
            description="Value when defending a city or structure."
          />
          <RatingCard
            label="Bear Hunt"
            value={hero.bear_tier}
            description="Value in Alliance Bear Hunt formations."
          />
          <RatingCard
            label="Rally joiner"
            value={hero.joiner_tier}
            description="Value when joining another player’s rally."
          />
        </div>
      </section>
      ...
      <section className="hero-companion-layout">
        <div className="hero-companion-main-column">
          <section className="hero-companion-panel">
            <div className="hero-companion-section__heading">
              <div>
                ...
                <h2>How to use {hero.name}</h2>
              </div>
            </div>
            <p className="hero-companion-copy">
              {hero.description ||
                'Detailed hero guidance has not yet been published.'}
            </p>
          </section>
          ...
          <section className="hero-companion-panel">
            <div className="hero-companion-section__heading">
              <div>
                <p className="eyebrow">Guide content</p>
                <h2>Progression and strategy</h2>
              </div>
            </div>
            ...
            <div className="hero-companion-guide-grid">
              <EmptyGuideSection
                title="Skills"
                description="Skill names, effects and upgrade priorities will appear here once connected to the published Hero Skills data."
              />
              <EmptyGuideSection
                title="Widget"
                description="Widget effects and recommended progression will appear here when published."
              />
              <EmptyGuideSection
                title="Hero pairings"
                description="Recommended hero combinations will be added through reviewed Companion content."
              />
              <EmptyGuideSection
                title="Formation guidance"
                description="Battle-role and formation recommendations will appear here when verified."
              />
            </div>
          </section>
        ...
        <aside className="hero-companion-sidebar">
          <section className="hero-companion-panel hero-companion-trust">
            ...
            <dl>
              <div>
                <dt>Source</dt>
                <dd>{hero.source_name || 'Not recorded'}</dd>
              </div>
              <div>
                <dt>Verification</dt>
                <dd>{hero.source_verified || 'Not recorded'}</dd>
              </div>
              <div>
                <dt>Source updated</dt>
                <dd>{formatDate(hero.source_updated_at)}</dd>
              </div>
              <div>
                <dt>Forge updated</dt>
                <dd>{formatDate(hero.updated_at)}</dd>
              </div>
              {hero.source_accuracy_score !== null && (
                <div>
                  <dt>Accuracy score</dt>
                  <dd>{hero.source_accuracy_score}%</dd>
                </div>
              )}
            </dl>
            ...
            {hero.source_url && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="hero-companion-source-link"
              >
    ...
    <main className="hero-companion-page">
      <section className="hero-companion-heading">
        <div>
          ...
          <h1>Heroes</h1>
          <p>
            Explore published hero ratings, roles and trusted Forge guidance.
          </p>
        </div>
        <div className="hero-companion-count">
          <strong>{heroes.length}</strong>
          <span>published heroes</span>
        </div>
      </section>
          ...
          >
            ...
            <div className="hero-companion-card__content">
              ...
              <div className="hero-companion-card__ratings">
                <span>Rally {hero.rally_tier || '—'}</span>
                <span>Bear {hero.bear_tier || '—'}</span>
              </div>
            </div>
          </Link>
        ))}
      ...
      {filteredHeroes.length === 0 && (
        <section className="hero-companion-empty">
          <h2>No heroes found</h2>
          <p>Try a different name, troop type, rarity or tag.</p>
        </section>
      )}
    </main>
  )
