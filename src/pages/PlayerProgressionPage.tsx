...
export default function PlayerProgressionPage() {
  ...
  const townCenterDisplay = canonicalTownCenterLevel === null ? 'Not available' : formatTownCenterRawLevel(canonicalTownCenterLevel)
  ...
  return <main className="player-progression-page">
    <header className="player-domain-hero"><div><p className="eyebrow">Player Domain</p><h1>Personal progression</h1><p>Track saved player snapshots with a compact linked-player context.</p></div><div className="player-domain-actions"><Link className="button button--secondary" to="/my-forge/profile">Edit Passport</Link><Link className="button button--secondary" to="/my-forge/heroes">Hero Showcase</Link></div></header>
    <section className="player-progress-summary"><article><span>Latest power</span><strong>{formatNumber(snapshots[0]?.currentPower ?? null)}</strong></article><article><span>Change</span><strong>{powerGain === null ? '—' : `${powerGain >= 0 ? '+' : ''}${powerGain.toLocaleString('en-GB')}`}</strong></article><article><span>Snapshots</span><strong>{snapshots.length}</strong></article></section>
