...
import { getMyPlayerProfile, type EditablePlayerProfile } from '../services/playerProfileService'
...
export default function PlayerProgressionPage() {
  ...
  const [profile,setProfile] = useState<EditablePlayerProfile|null>(null)
  ...
  useEffect(() => { let cancelled=false; async function load(){ if(authLoading)return; if(!user){setLoading(false);return} try { const {data,error:identityError}=await supabase.from('profiles').select('forge_id').eq('id',user.id).single(); if(identityError)throw identityError; const result=await getMyPlayerProfile(user.id,data.forge_id); if(!result){if(!cancelled)setProfile(null);return} const history=await getMyProgression(result.playerAccountId); if(!cancelled){setProfile(result);setSnapshots(history);const latest=history[0];if(latest)setForm({currentPower:latest.currentPower,highestPower:latest.highestPower,townCenterLevel:latest.townCenterLevel,truegoldLevel:latest.truegoldLevel,vipLevel:latest.vipLevel,infantryTier:latest.infantryTier,lancerTier:latest.lancerTier,marksmanTier:latest.marksmanTier,governorGearScore:latest.governorGearScore,governorCharmScore:latest.governorCharmScore,notes:null,isPublic:latest.isPublic})}} catch(caught){if(!cancelled)setError(caught instanceof Error?caught.message:'Progression could not be loaded.')} finally{if(!cancelled)setLoading(false)}} void load(); return()=>{cancelled=true} },[authLoading,user])
  ...
  async function save(event:FormEvent){event.preventDefault();if(!profile)return;setSaving(true);setError('');setMessage('');try{await addProgressionSnapshot(profile.playerAccountId,form);setSnapshots(await getMyProgression(profile.playerAccountId));setMessage('Progression snapshot saved.')}catch(caught){setError(caught instanceof Error?caught.message:'Progression could not be saved.')}finally{setSaving(false)}}
  ...
  if(!profile)return <main className="player-domain-state"><h1>Link a player first</h1><p>A primary Kingshot player account is required.</p><Link className="button button--primary" to="/my-forge">Open My Forge</Link></main>
