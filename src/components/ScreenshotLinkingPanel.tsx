...
export default function ScreenshotLinkingPanel({ onCandidate, onReview }: { onCandidate: (playerId: string) => void; onReview?: (review: AccountLinkOcrReview | null) => void }) {
  ...
  async function handleFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const initialReview: AccountLinkOcrReview = { playerId: valueFor('playerId'), displayName: valueFor('displayName'), kingdom: valueFor('kingdom'), allianceTag: valueFor('allianceTag'), townCenterLevel: '', townCenterSuggestion: valueFor('townCenterLevel') || undefined, evidenceId, confidence: data.engineConfidence ?? 0, source: 'Screenshot OCR', userConfirmed: { playerId: false, displayName: false, kingdom: false, allianceTag: false, townCenterLevel: false } }
  ...
  return <section className="linked-player-screenshot" aria-labelledby="screenshot-linking-title">
    ...
    {result && review && <div className="linked-player-screenshot__result"><strong>OCR review form</strong><p>Source: {review.source}. OCR does not verify ownership. Corrected fields are marked user-confirmed.</p>{(['playerId', 'displayName', 'kingdom', 'allianceTag', 'townCenterLevel'] as const).map((field) => { const labels = { playerId: 'Player ID', displayName: 'Name', kingdom: 'Kingdom', allianceTag: 'Alliance', townCenterLevel: 'Town Centre Level' }; const candidate = result.candidates.find((item) => item.field === field); const isTown = field === 'townCenterLevel'; return <div key={field} className="linked-player-screenshot__candidate"><label htmlFor={`ocr-${field}`}>{labels[field]}</label><input id={`ocr-${field}`} type={isTown ? 'number' : 'text'} min={isTown ? 1 : undefined} max={isTown ? 30 : undefined} step={isTown ? 1 : undefined} required={isTown} value={review[field]} placeholder={isTown ? 'Enter the level shown on your profile card' : undefined} onChange={(event) => { const next = { ...review, [field]: event.target.value, userConfirmed: { ...review.userConfirmed, [field]: true } }; setReview(next); onReview?.(next); if (field === 'playerId') onCandidate(event.target.value) }} />{isTown && review.townCenterSuggestion && <small>OCR suggestion: {review.townCenterSuggestion} — verify against the screenshot. This suggestion is not prefilled or trusted.</small>}<small>{candidate ? `${isTown ? 'Screenshot OCR — review required' : candidate.confidence >= 0.85 ? 'Screenshot OCR — recognised' : 'Screenshot OCR — review required'} · ${candidate.source} · ${candidate.mappingVersion}` : isTown ? 'Manual confirmation required · User confirmed after entry' : 'Could not read'}{review.userConfirmed[field] ? ' · user-confirmed' : ''}{candidate?.warnings.length ? ` · ${candidate.warnings.join(' ')}` : ''}</small></div> })}</div>}
  </section>
}
