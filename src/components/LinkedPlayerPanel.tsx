...
function getVerificationLabel(status: PlayerAccount['verification_status']) {
  switch (status) {
    case 'verified': return 'Verified player'
...
function getVerificationDescription(status: PlayerAccount['verification_status']) {
  switch (status) {
    ...
    case 'rejected': return 'The previous verification request was not approved.'
    case 'revoked': return 'Verification for this player account has been removed.'
    default: return 'The player data has been linked, but ownership has not yet been verified.'
  }
}
...
function LinkedPlayerPanel() {
  ...
  function validateInputs() {
    ...
    if (!/^\d{1,20}$/u.test(cleanedPlayerId)) throw new Error('Enter a valid Kingshot Player ID using numbers only.')
    if (!/^\d{1,4}$/u.test(cleanedKingdomId) || Number(cleanedKingdomId) < 1 || Number(cleanedKingdomId) > 9999) throw new Error('Enter a valid Kingshot State between 1 and 9999.')
  ...
  async function handleOcrFallbackSave() {
    ...
    try {
      ...
      setOcrReview(null)
      setMessage('Screenshot details saved as an unverified linked account.')
      notifyPlayerIdentityChanged()
      await refreshPlayerIdentity()
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : 'The OCR fallback could not be saved safely.')
    } finally {
  ...
  async function handleRemoveAccount() {
    ...
    if (!['linked', 'pending', 'rejected'].includes(linkedAccount.verification_status)) {
      setErrorMessage('Verified player accounts must be reviewed before they can be removed.')
      return
    }
    if (!window.confirm('Remove this linked Kingshot account from your Forge profile?')) return

    setRemoving(true)
    setMessage('')
    setErrorMessage('')
    const { error } = await supabase.from('player_accounts').delete().eq('id', linkedAccount.id).eq('user_id', user.id)
    if (error) {
  ...
  async function handlePrivacyChange(isPublic: boolean) {
    if (!user || !linkedAccount) return
    setErrorMessage('')
    const { error } = await supabase.from('player_accounts').update({ is_public: isPublic, updated_at: new Date().toISOString() }).eq('id', linkedAccount.id).eq('user_id', user.id)
    if (error) {
      setErrorMessage(error.message)
      return
    }
  ...
  if (authLoading || loadingAccount) return <section className="linked-player-panel"><p>Loading Kingshot account…</p></section>
  if (playerIdentityError) return <section className="linked-player-panel" role="alert"><p>{playerIdentityError}</p><button type="button" className="button button--secondary" onClick={() => void refreshPlayerIdentity()}>Try again</button></section>
  if (!user) return <section className="linked-player-panel linked-player-panel--signed-out"><span>🎮</span><div><p className="eyebrow">Kingshot identity</p><h2>Link your Kingshot player</h2><p>Sign in with Google first, then connect your Player ID and State.</p></div></section>
  ...
  const apiConflicts = previewPlayer && ocrReview
    ? [
      ...
      String(previewPlayer.kingdom) !== ocrReview.kingdom ? 'State' : null,
  ...
  return (
    <section className="linked-player-panel">
      ...
      {linkedAccount && (
        <article className="linked-player-card">
          <div className="linked-player-card__identity">
            ...
            <div><span className="linked-player-card__label">Primary Kingshot player</span><h3>{linkedAccount.player_name}</h3><p>{linkedAccount.level_rendered_detailed || linkedAccount.level_rendered || (linkedAccount.player_level ? `Level ${linkedAccount.player_level}` : 'Level unavailable')}</p></div>
          </div>
      ...
      {errorMessage && <p className="linked-player-message linked-player-message--error">{errorMessage}</p>}
    </section>
  )
}
