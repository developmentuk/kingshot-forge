...
import type { RenderProfile } from '../../../shared/domains/art-studio/rendering'
import { getRenderGlyphWidth, RENDER_PROFILES } from '../../../shared/domains/art-studio/rendering'
...
function KingshotGrid({ artwork, lines, classes, labelledBy, calibration, renderProfile, device }: { artwork: string; lines: string[]; classes: string; labelledBy?: string; calibration: CalibrationConfiguration; renderProfile: RenderProfile; device: DeviceProfile }) {
  ...
  return <div className={classes} role="img" aria-labelledby={labelledBy} aria-label={labelledBy ? undefined : 'Fixed-cell artwork preview'} data-source-text={normaliseArtwork(artwork)}>
    {grid.map((row) => <div className="kingshot-cell-grid__row" key={row.row} aria-hidden="true" style={{ minHeight: `${device.cellHeight * renderProfile.lineHeight}px` }}>
      {row.cells.length === 0 ? <span className="kingshot-cell-grid__cell kingshot-cell-grid__cell--space" style={{ width: `${device.cellWidth}px`, minWidth: `${device.cellWidth}px`, flexBasis: `${device.cellWidth}px` }}>&nbsp;</span> : row.cells.map((cell) => {
        const paint = calibration[cell.family]
        const width = device.cellWidth * getRenderGlyphWidth(cell.glyph, renderProfile)
        return <span className={`kingshot-cell-grid__cell kingshot-cell-grid__cell--${cell.family}`} style={{ width: `${width}px`, minWidth: `${width}px`, flexBasis: `${width}px` }} data-grid-column={cell.column} data-grid-row={cell.row} key={`${row.row}-${cell.column}`}><span className="kingshot-cell-grid__glyph" style={{ '--forge-glyph-scale': paint.glyphScale, '--forge-glyph-scale-x': paint.horizontalScale, '--forge-glyph-scale-y': paint.verticalScale, '--forge-baseline-offset': `${paint.baselineOffset + renderProfile.baseline}px`, '--forge-glyph-family': paint.fontFamily, '--forge-glyph-weight': paint.fontWeight } as React.CSSProperties}>{cell.glyph === ' ' ? '\u00a0' : cell.glyph}</span></span>
      })}
    </div>)}
  </div>
}
...
export function KingshotArtRenderer({ artwork, mode = 'kingshot', compact = false, className = '', maxLines, labelledBy, profile = 'auto', deviceProfile, deviceProfileConfig, calibration = DEFAULT_CALIBRATION, renderProfile = RENDER_PROFILES['kingshot-chat'], showSimulation }: KingshotArtRendererProps) {
  ...
  const artworkNode = <KingshotGrid artwork={artwork} lines={lines} classes={classes} labelledBy={labelledBy} calibration={calibration} renderProfile={renderProfile} device={profileData} />
