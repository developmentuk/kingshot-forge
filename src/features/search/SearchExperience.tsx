...
export function ForgeConnections({ dataset, id, limit = 12, showPanels = true }: { dataset: string; id: string; limit?: number; showPanels?: boolean }) {
  ...
  const related = [...new Map(matches.filter((match) => `${match.record.dataset}:${match.record.id}` !== `${dataset}:${id}` && resolveSearchDestination(match.record)).map((match) => [`${match.record.dataset}:${match.record.id}`, match])).values()].sort((left, right) => (right.score - left.score) || left.record.title.localeCompare(right.record.title)).slice(0, limit)
