...
export function ForgeConnections({ dataset, id, limit = 12, showPanels = true }: { dataset: string; id: string; limit?: number; showPanels?: boolean }) {
  ...
  const related = [...new Map(matches.filter((match) => `${match.record.dataset}:${match.record.id}` !== `${dataset}:${id}` && match.record.status === 'published' && (match.relationshipType || (match.record.tags?.length ?? 0) > 0) && resolveSearchDestination(match.record)).map((match) => [`${match.record.dataset}:${match.record.id}`, match])).values()].sort((left, right) => (right.score - left.score) || left.record.title.localeCompare(right.record.title)).slice(0, limit)
 ...
 return <section className="forge-connections" aria-labelledby="forge-connections-title"><div className="forge-connections__heading"><div><p className="forge-search__eyebrow">Relationship Engine</p><h2 id="forge-connections-title">Forge Connections</h2><p>Curated published content connected by editorial relationships and canonical tags.</p></div><Link to={`/search?relationshipFrom=${encodeURIComponent(`${dataset}:${id}`)}`}>View all related content</Link></div>{groups.map((group) => <div className="forge-connections__group" key={group}><h3>{group}</h3><div className="forge-connections__grid">{related.filter((match) => relationshipLabel(match.relationshipType) === group).map((match) => { const { record } = match; const confidence = confidenceLabel(record); const destination = resolveSearchDestination(record); return destination ? <Link className="forge-connection-card" key={`${record.dataset}:${record.id}`} to={destination}><span className="forge-result__icon" aria-hidden="true">◈</span><span><span className="forge-connection-card__meta"><small>{datasetLabel(record.dataset)}</small>{confidence && <small className="forge-confidence">{confidence}</small>}</span><strong>{record.title}</strong>{record.subtitle && <small>{record.subtitle}</small>}<em>{record.summary ?? 'Published related content'}</em>{record.tags?.length ? <small className="forge-connection-card__tags">{record.tags.slice(0, 4).join(' · ')}</small> : null}<small className="forge-connection-card__why">{explain(match)}</small><span className="forge-connection-card__open">Open →</span></span></Link> : null})}</div></div>)}{showPanels && <KnowledgePanels matches={related} />}</section>
}
