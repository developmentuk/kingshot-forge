...
async function readFixture(file: File): Promise<LoadedFixture> {
  ...
  return { filename: file.name, source, bytes: raw.byteLength, hash, lineEndings: lineEndingFormat(raw), diagnostics: analyseText(source) }
}

function metadata(fixture: LoadedFixture | null) {
  if (!fixture) return null
  return [
    ['Filename', fixture.filename], ['Raw bytes', fixture.bytes], ['Line endings', fixture.lineEndings], ['SHA-256', fixture.hash],
    ['Canonical hash', fixture.hash === CANONICAL_HASH ? 'MATCH' : 'MISMATCH'], ['Lines', fixture.diagnostics.lineCount],
...
function AcceptanceModal({ source, onClose }: { source: string; onClose: () => void }) {
  ...
  return <div className="art-preview-modal" role="dialog" aria-modal="true" aria-labelledby="art-acceptance-modal-title">
    ...
    <div className="art-preview-modal__panel" ref={panelRef}>
      ...
      <p className="art-library-card__creator">Created by Acceptance Fixture · calibration_required</p>
      ...
      <div className={`art-preview-modal__content art-preview-modal__content--${mode}`}><div className="art-preview-stage"><KingshotArtRenderer artwork={source} mode={mode} labelledBy="art-acceptance-modal-title" /></div></div>
      <div className="art-preview-modal__details"><span>{analyseText(source).graphemeCount} graphemes</span><span>{analyseText(source).lineCount} lines</span><span>large</span><span>calibration_required</span><span>Acceptance fixture</span></div>
    </div>
  </div>
}
...
export default function ArtStudioAcceptancePage() {
  ...
  return <main className="art-acceptance-page" onDragOver={(event) => event.preventDefault()} onDrop={handleDrop}>
    <section className="art-acceptance-hero"><p className="eyebrow">Development/test-only · ART-003</p><h1>Art Studio deterministic acceptance harness</h1><p>Load the owner’s raw fixture and reference captures into the real Forge renderer without authentication, Supabase, persistence or production route exposure.</p><div className="art-acceptance-warning" role="note">Fixture status: <strong>calibration_required</strong>. This harness provides comparison evidence; it does not claim pixel-perfect parity automatically.</div></section>
    <section className="art-acceptance-loader art-acceptance-panel"><div><h2>1. Load canonical fixture</h2><p>Raw bytes are hashed before UTF-8 decoding. CRLF is preserved in the source used by every surface and copy action.</p></div><label className="art-acceptance-file-picker"><span>Choose text fixture</span><input type="file" accept="text/plain,.txt" onChange={(event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (file) void loadFixture(file) }} /></label><div className="art-acceptance-dropzone" onDrop={handleDrop}>Drop <code>{CANONICAL_FILENAME}</code> here</div>{error && <p className="error-state" role="alert">{error}</p>}{fixture && <dl className="art-acceptance-metadata">{metadata(fixture)?.map(([label, value]) => <div key={label}><dt>{label}</dt><dd className={label === 'Canonical hash' ? (value === 'MATCH' ? 'is-match' : 'is-mismatch') : ''}>{String(value)}</dd></div>)}</dl>}</section>
    ...
    <section className="art-acceptance-surfaces"><div className="art-acceptance-section-heading"><p className="eyebrow">3. Shared presentation surfaces</p><h2>All surfaces use the same source and renderer</h2><p>{fixture ? `Source hash ${fixture.hash} · profile Kingshot chat bubble · fixture status calibration_required` : 'Load the canonical fixture to enable visual surfaces.'}</p></div>
      <Surface title="Art Studio editor preview"><label className="art-acceptance-editor">Source text<textarea value={source} readOnly spellCheck="false" /></label>{source ? <KingshotArtRenderer artwork={source} labelledBy="art-acceptance-editor-title" /> : <p className="art-acceptance-empty">Load the fixture first.</p>}</Surface>
      <Surface title="Gallery card"><article className="art-library-card art-acceptance-gallery-card"><div className="art-library-card__heading"><div><span className="art-library-card__category">Cats · calibration_required</span><h3>Wow I’m So Cute</h3></div><span>Acceptance Fixture</span></div><p className="art-library-card__description">Canonical owner fixture rendered through the gallery card geometry.</p><button ref={openerRef} type="button" className="art-card-preview-button" onClick={() => setModalOpen(true)} aria-label="Preview Wow I’m So Cute"><KingshotArtRenderer artwork={source} compact maxLines={8} /><span>Tap to view full artwork</span></button><div className="art-library-card__metadata"><span>{diagnostics.graphemeCount} graphemes</span><span>large</span><span>calibration_required</span></div></article></Surface>
    ...
    {modalOpen && source && <AcceptanceModal source={source} onClose={() => { setModalOpen(false); openerRef.current?.focus({ preventScroll: true }) }} />}
  </main>
}
