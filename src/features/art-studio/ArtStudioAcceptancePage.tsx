...
export default function ArtStudioAcceptancePage() {
  ...
  return <main className="art-acceptance-page" onDragOver={(event) => event.preventDefault()} onDrop={handleDrop}>
    ...
    <Surface title="Gallery card"><article className="art-library-card art-acceptance-gallery-card"><div className="art-library-card__heading"><div><span className="art-library-card__category">Cats · calibrated</span><h3>Wow I’m So Cute</h3></div><span>Acceptance Fixture</span></div><p className="art-library-card__description">Canonical owner fixture rendered through the gallery card geometry.</p><button ref={openerRef} type="button" className="art-card-preview-button" onClick={() => setModalOpen(true)} aria-label="Preview Wow I’m So Cute"><KingshotArtRenderer artwork={source} compact maxLines={8} /><span>Tap to view full artwork</span></button><div className="art-library-card__metadata"><span>{diagnostics.graphemeCount} graphemes</span><span>large</span><span>calibrated</span></div></article></Surface>
