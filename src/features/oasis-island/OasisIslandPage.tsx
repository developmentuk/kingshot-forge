...
import { imageForBuilding, formatFootprint, searchTextForBuilding, trustLabel, formatPercent, type OasisBuilding } from './oasisIslandData'
...
function BuildingDetail({ building }: { building: OasisBuilding }) {
  ...
  return <main className="oasis-page oasis-page--detail">
    ...
    {building.verification?.current?.status === 'owner_direct_ingame_verified' ? null : null}
  </main>
}
...
export default function OasisIslandPage() {
  ...
  const buildings = useMemo(() => [] as OasisBuilding[], [])
  const loading = false
  const error = ''
  ...
  const visible = useMemo(() => buildings.filter((building) => { const haystack = searchTextForBuilding(building); return haystack.includes(query.toLocaleLowerCase()) && (rarity === 'all' || building.rarity === rarity) }), [buildings, query, rarity])
  ...
  return <main className="oasis-page"><header className="oasis-hero"><div><p className="eyebrow">Kingshot Forge · Companion / Oasis Island</p><h1>Oasis Island, made understandable</h1><p>Explore every known Oasis Island building, see what each level gives you, and find the upgrades that matter for your account.</p><div className="oasis-hero__actions"><Link className="button button--primary" to="/calculators/island-chest-route-optimizer">Plan chest routes</Link><a className="button button--secondary" href="#guide">Read the guide</a></div></div><div className="oasis-hero__facts"><strong>{loading ? '…' : error ? '—' : buildings.length}</strong><span>buildings and structures</span><small>Chest Route Optimizer included</small></div></header>
    <section className="oasis-catalogue" aria-labelledby="oasis-catalogue-title"><div className="oasis-section-heading"><div><p className="eyebrow">Building guide</p><h2 id="oasis-catalogue-title">Find a structure</h2></div><span>{loading ? 'Loading buildings…' : error ? 'Buildings unavailable' : `${visible.length} of ${buildings.length} buildings`}</span></div><div className="oasis-controls"><label><span>Search</span><input type="search" value={query} onChange={(event) => { const next = new URLSearchParams(params); if (event.target.value) next.set('q', event.target.value); else next.delete('q'); setParams(next, { replace: true }) }} placeholder="Search buildings, bonuses or aliases…" /></label><label><span>Rarity</span><select value={rarity} onChange={(event) => { const next = new URLSearchParams(params); if (event.target.value === 'all') next.delete('rarity'); else next.set('rarity', event.target.value); setParams(next, { replace: true }) }}><option value="all">All rarities</option>{rarities.map((value) => <option key={value} value={value}>{value}</option>)}</select></label></div>{error && <div className="oasis-state oasis-state--error" role="alert"><p>{error}</p></div>}{loading && <p className="oasis-state" role="status">Loading Oasis buildings…</p>}{!loading && !error && <div className="oasis-building-grid">{visible.map((building) => <Link className="oasis-building-card" to={`/oasis-island/buildings/${building.id}`} key={building.id}><BuildingImage building={building} compact /><div><span className="oasis-building-card__meta">{building.rarity ?? 'Structure'} · {trustLabel(building.verification?.current?.status)}</span><h3>{building.name}</h3><p>{building.function ?? 'See levels, bonuses and planning details for this building.'}</p><small>{building.levels.length ? `${building.levels.length} levels` : 'More details have not been recorded yet.'} · View details →</small></div></Link>)}</div>}</section>
    <div id="guide"><OasisGuide /></div>
    <footer className="oasis-footer"><strong>Publication status: pending.</strong><span>Reviewed source evidence stays private until an approved published projection is available.</span></footer>
  </main>
}
