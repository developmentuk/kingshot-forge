...
export function VisionAccountLinkingAcceptancePage() {
  ...
  return <main className="vision-acceptance" aria-labelledby="vision-acceptance-title">
    ...
    <section className="vision-acceptance__panel">
      ...
      {result && <div className="vision-acceptance__results"><h2>{result.candidates.length ? 'Extracted candidates' : 'Could not reliably read this image'}</h2>{(['playerId', 'displayName', 'kingdom'] as const).map((field) => { const candidate = result.candidates.find((item) => item.field === field); const label = field === 'playerId' ? 'Player ID' : field === 'displayName' ? 'Name' : 'Kingdom'; return <div className="vision-acceptance__candidate" key={field}><span>{label}</span>{candidate ? <><strong>{candidate.value}</strong><small>{candidate.confidence >= 0.85 ? 'High confidence' : candidate.confidence >= 0.6 ? 'Review confidence' : 'Low confidence'} · {candidate.confidence.toFixed(2)}</small></> : <><strong>Could not read</strong><small>Try a clearer profile card or enter this field manually.</small></>}</div> })}<p className="vision-acceptance__provenance">Runtime: {result.provenance.pluginKey} · plugin {result.provenance.pluginVersion} · engine {result.provenance.engineVersion}</p></div>}
