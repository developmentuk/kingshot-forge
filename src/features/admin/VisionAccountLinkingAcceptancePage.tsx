...
export function VisionAccountLinkingAcceptancePage() {
  ...
  function handleFileSelection(event: { currentTarget: HTMLInputElement }) {
    ...
    setError(''); setResult(null); setIntentId(null); setEvidenceId(null)
    if (previewUrl) URL.revokeObjectURL(previewUrl)
    if (!file) { setSelectedFile(null); setPreviewUrl(null); setState('error'); setError('No file was selected.'); return }
  ...
  return <main className="vision-acceptance" aria-labelledby="vision-acceptance-title">
    ...
    <section className="vision-acceptance__panel">
      ...
      {(result || intentId) && <button className="button button--secondary" type="button" disabled={busy} onClick={() => void cancel}>Cancel and delete evidence</button>}
    </section>
  </main>
}
