...
import { KINGSHOT_PROFILE_V5_REGIONS } from '../../../shared/domains/player-identity/kingshotProfileMapping'
...
export function VisionAccountLinkingAcceptancePage() {
  ...
  return <main className="vision-acceptance" aria-labelledby="vision-acceptance-title">
    ...
    <section className="vision-acceptance__panel">
      ...
      {previewUrl && <><button className="button button--secondary vision-acceptance__toggle" type="button" aria-pressed={showRegions} onClick={() => setShowRegions((visible) => !visible)}>{showRegions ? 'Hide OCR regions' : 'Show OCR regions'}</button><div className="vision-acceptance__preview-wrap"><img className="vision-acceptance__preview" src={previewUrl} alt="Selected screenshot preview" />{showRegions && <div className="vision-acceptance__overlay" aria-label="OCR region overlay">{KINGSHOT_PROFILE_V5_REGIONS.map((region, index) => <div key={region.key} className={`vision-acceptance__overlay-region vision-acceptance__overlay-region--${region.key} ${region.componentRole === 'exclusion' ? 'vision-acceptance__overlay-region--exclusion' : ''}`} style={{ left: `${region.x * 100}%`, top: `${region.y * 100}%`, width: `${region.width * 100}%`, height: `${region.height * 100}%` }}><span>{index + 1}. {region.label}</span></div>)}</div>}</div></>}
