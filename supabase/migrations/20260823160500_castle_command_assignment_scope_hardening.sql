...
-- Assignment mutations must honour the exact alliance-scoped sharing consent,
-- and closed-session roster history is immutable.
...
declare
  command_session public.castle_command_sessions%rowtype;
begin
  select session.* into command_session
  ...
  if command_session.status = 'closed' then
    raise exception 'Closed Castle Command session assignments are immutable' using errcode = '22023';
  end if;
