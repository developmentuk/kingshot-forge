...
begin
  ...
  if p_apply_alliance_authority then
    ...
    if authority_state.player_account_id is null
      and current_membership.id is not null
      and current_membership.updated_at is not null
      and p_authority_observed_at <= current_membership.updated_at then
    ...
    elsif nullif(btrim(p_alliance_tag), '') is not null
